...
public class TicketAdapter extends RecyclerView.Adapter<TicketAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.xjumlah_tiket.setText(myTicket.get(position).getJumlah_tiket());

        String getNamaWisata = myTicket.get(position).getNama_wisata();

        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent gotomticketdetails = new Intent(context, MyTicketDetailAct.class);
                gotomticketdetails.putExtra("nama_wisata", getNamaWisata);
                context.startActivity(gotomticketdetails);
            }
        });
    }
